...
export class ToolResolverBase {
  ...
  @graphql.ResolveField(() => [ToolUse], { name: "toolUses" })
  async findToolUses(
    @graphql.Parent() parent: Tool,
    @graphql.Args() args: ToolUseFindManyArgs
  ): Promise<ToolUse[]> {
    const results = await this.service.findToolUses(parent.id, args);
  ...
  @graphql.ResolveField(() => Llm, {
    nullable: true,
    name: "creator",
  })
  async getCreator(@graphql.Parent() parent: Tool): Promise<Llm | null> {
