/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LlmConfigurationWhereInput } from "./LlmConfigurationWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { LlmConfigurationOrderByInput } from "./LlmConfigurationOrderByInput";

@ArgsType()
class LlmConfigurationFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => LlmConfigurationWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => LlmConfigurationWhereInput, { nullable: true })
  @Type(() => LlmConfigurationWhereInput)
  where?: LlmConfigurationWhereInput;

  @ApiProperty({
    required: false,
    type: [LlmConfigurationOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [LlmConfigurationOrderByInput], { nullable: true })
  @Type(() => LlmConfigurationOrderByInput)
  orderBy?: Array<LlmConfigurationOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { LlmConfigurationFindManyArgs as LlmConfigurationFindManyArgs };
